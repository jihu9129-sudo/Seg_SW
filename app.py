...
# ======================= 설정 ==========================
DEFAULT_MODEL_PATH = "student_model.pth"
IMG_SIZE = 256
...
@st.cache_resource
def load_model(file=None, path=DEFAULT_MODEL_PATH):
    m = UNetImproved().to(device)
    if file is not None:
        try:
            state = torch.load(file, map_location=device)
            m.load_state_dict(state)
            st.success("업로드 모델 로드 성공")
        except Exception as e:
            st.warning(f"업로드 모델 로드 실패 ({e}) — 랜덤 초기화 모델 사용")
    elif os.path.exists(path):
        try:
            state = torch.load(path, map_location=device)
            m.load_state_dict(state)
            st.success(f"기본 모델 로드 성공: {path}")
        except Exception as e:
            st.warning(f"기본 모델 로드 실패 ({e}) — 랜덤 초기화 모델 사용")
    else:
