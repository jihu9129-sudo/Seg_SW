...
@st.cache_data
def preprocess_image(img, apply_hair, apply_clahe, clahe_clip, clahe_tile, apply_blur, blur_ksize, apply_bc, alpha_bc, beta_bc):
    img_proc = img.copy()
...
img_proc = preprocess_image(img_orig, apply_hair, apply_clahe, clahe_clip, clahe_tile, apply_blur, blur_ksize, apply_bc, alpha_bc, beta_bc)

# --- 모델 입력 및 추론
img_resized = np.array(Image.fromarray(img_proc).resize((IMG_SIZE, IMG_SIZE)))

@st.cache_data
def predict_mask_from_array(img_array):
    x_tensor = transforms.ToTensor()(img_array).unsqueeze(0).to(device)
    with torch.no_grad():
        out = model(x_tensor)
        prob = torch.sigmoid(out).squeeze().cpu().numpy()
    return prob

prob = predict_mask_from_array(img_resized)
